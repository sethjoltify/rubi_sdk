...
class CbridgeCrossChainProvider extends cross_chain_provider_1.CrossChainProvider {
    ...
    async calculate(fromToken, toToken, options) {
        ...
        try {
            ...
            const { amount, maxSlippage, fee } = await this.getEstimates(transitToken, toToken, options, config);
            if (!amount) {
            ...
            const gasData = options.gasCalculation === 'enabled'
                ? await cbridge_cross_chain_trade_1.CbridgeCrossChainTrade.getGasData(fromToken, to, onChainTrade)
                : null;
            ...
            return {
                trade: new cbridge_cross_chain_trade_1.CbridgeCrossChainTrade({
                    ...
                    feeInfo: {
                        ...
                        provider: {
                            cryptoFee: {
                                amount: web3_pure_1.Web3Pure.fromWei(fee, toToken.decimals),
