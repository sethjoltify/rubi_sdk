...
class SquidrouterCrossChainTrade extends evm_cross_chain_trade_1.EvmCrossChainTrade {
    ...
    async swapDirect(options = {}) {
        ...
        let transactionHash;
        try {
            const { route: { transactionRequest: { data, value, targetAddress } } } = await this.transactionRequest(options?.receiverAddress || this.walletAddress);
    ...
    async getContractParams(options) {
        const { route: { transactionRequest: { data, value: providerValue, targetAddress } } } = await this.transactionRequest(options?.receiverAddress || this.walletAddress);
        ...
        const providerData = await proxy_cross_chain_evm_trade_1.ProxyCrossChainEvmTrade.getGenericProviderData(targetAddress, data, this.fromBlockchain, targetAddress, extraNativeFee);
